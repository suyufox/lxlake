...
| `script`     | **二层基础类**：脚本的 IR 与求值器——**规划中，未实装**                       | JS 与 Lua **双后端并行**（见「求值目标选型」）；机制在引擎，脚本内容在应用 |
...
| `capability` | **二层能力封装**，一个能力一个模块：`webview` 已落位，`cli` 新增（启动参数） | 逐个 feature 隔离重依赖，能力以**查询**形式暴露                            |
...
cli-args    = ["dep:clap"]                     # 启动参数解析：CEF 子进程分型 + 应用自定义参数
script-js   = ["dep:boa_engine"]               # JS 求值器（boa，纯 Rust）
script-lua  = ["dep:mlua"]                     # Lua 求值器（mlua vendored，需 C 编译器）；可与 script-js 并存
notify      = ["dep:windows"]                  # Windows 上发真 toast 要 WinRT；其余平台降级
```
...
### 求值目标选型：JS 与 Lua 双后端并行

脚本要服务两件事：**内容在应用**（mod / 自定义逻辑）与**编辑器积木 / 代码双线的公共后端**。
结论先给：**语言收 JS 与 Lua 两门、并行支持**；不选 C#，WASM 只走插件通道。

先把两个决定分开——上一版把它们绑在一起了：

- **语言**（作者写什么）：JS 与 Lua **两门都要**
- **引擎**（谁解释它）：JS 走 `boa`（纯 Rust，零 C 依赖）；Lua 走 `mlua`（`vendored`，由 `cc` 编源码）

| 候选 | 结论     | 理由                                                                  |
| ---- | -------- | --------------------------------------------------------------------- |
| JS   | **采用** | 用户基数最大，编辑器「代码式」线的语法与工具不必自造；`boa` 零 C 依赖 |
| Lua  | **采用** | 游戏内容脚本的事实标准；`mlua` 的 `vendored` 让构建不依赖系统 Lua     |
| C#   | 不采用   | 宿主 CLR 或 .NET AOT 都牵动运行时模型，P/Invoke 桥接层重              |
| WASM | 留给插件 | 窄 ABI、版本化是**插件**要的，不是内容脚本要的                        |

三条口径：

1. **两门语言不是两套 API**。绑定到 `ecs` 的宿主函数（entity / component / resource 那组）**只定义
   一次**，两个后端各自生成各自的绑定。这是双语言并行的**唯一成本控制点**——若各写一份宿主 API，
   两份必然漂移
2. **引擎藏在 trait 后面**。`script::Evaluator` 是接口，`boa` 与 `mlua` 是两个实装。语言是稳定的，
   引擎不是：JS 侧若要性能余量可换 `rquickjs`（ES2020，需 C 编译器），Lua 侧若要 JIT 可换 `luajit`
   ——换引擎不动脚本源码
3. **不复用 CEF 的 V8**。CEF 已经把 V8 带进了进程，但把「能不能跑脚本」绑在「要不要开 CEF 纹理
   模式」上是错的：脚本是框架主线随时可用的能力

**两条边界要分开写死**：

- **脚本通道（JS / Lua）**：内容侧、mod、编辑器求值后端。在 `script` 模块里，跑在**主线程或专用
  worker**，能直接拿 `ecs` 的可变引用。代价是脚本崩了进程跟着崩——内容由作者负责（Lua 侧有 `pcall`
  可挡一层，JS 侧没有）
- **插件通道（WASM / wit-bindgen）**：第三方、沙箱、版本化边界。走 `plugin` 模块，一条窄 C ABI。
  脚本与插件**不是同一条路**，不要混成「都是跑外部代码」

**编辑器双线因此降到 IR，不降到某门语言**：积木出的图与代码出的语句都先落到**中立的 IR**，再由 IR
按目标后端生成 JS 或 Lua 源码。项目选哪门语言写在清单 `lxlake.toml` 里（项目级设置），编辑器按它
决定 codegen；应用侧的高级用户也可以绕过 IR，直接给 JS / Lua 源码。

feature 两档**可并存**：`script-js`（`boa_engine`）与 `script-lua`（`mlua` + `vendored`）。
`#[lxlake::entry]` 生成的入口里**不**做脚本初始化——脚本由应用在 `Builder` 装配期声明，`run` 时按需
建求值器。
...
| 启动参数       | **全缺**（`capability::cli` 未建）                                      | 框架能力：**第一为 CEF**（子进程靠命令行分型：`--type=renderer` / `--type=gpu` 等，必须由框架统一解析并交给 CEF，应用不手搓 argv），**第二给应用留自定义启动参数**（调试开关、覆盖配置、路径重定位）。接口常编译，`clap` 关在 `cli-args` feature 后 |
...
| 横切能力       | `capability` 下只有 `webview` 一个                                      | `cli`（启动参数，本轮新入册，为 CEF 子进程分型与应用自定义参数）、`notify`（二层的规矩已立，代码未建）、`media`（媒体解码）；**更新**已单列一行                                                                                                     |
...
| 脚本     | **零**；`src/script/` 不存在                                               | **基础类（二层）**：IR 与求值器；**JS（boa）与 Lua（mlua）双后端并行**（见「求值目标选型」），积木 / 代码双线降到中立 IR，再由 IR 生成目标语言源码                   |
...
cli-args（clap，启动参数解析）──→ CEF 子进程分型（`--type=` 等由框架统一解析后递给 CEF）

配置与持久化 ──→ 键位覆盖 / 窗口位置 / 终端用户配置（与存档同源不同库）

求值目标已定（JS / Lua 双后端）──→ 代码语言与积木语言 ──→ 脚本插件边界 ──→ 插件宿主
...
5. **求值目标已定：JS 与 Lua 双后端并行**（见「求值目标选型」），于是「先定求值目标」这一格已消。
   剩下的顺序是：**先定脚本插件边界，最后才谈插件宿主**。反过来做（先挑 wasmtime、先写 C ABI）会返工：
   边界形状由 IR 与求值器决定，不由宿主运行时决定。脚本通道（JS / Lua）与插件通道（WASM）是两条路，
   别混成「都是跑外部代码」
