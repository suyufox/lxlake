...
- 纹理模式可参与 z 序与裁剪，取帧 → 纹理（**这一步的接口还不存在**，见「缺口清单」）
...
| 命令总线   | `runtime::command`（总线管序、执行器管落地）                            | **没有实体存储**：`EntityCommand` 在契约层就位却无人执行           |
...
| 平台入口   | windows / linux / macos / android 四 target（`platform/mod.rs` 分档）   | **ios 后端**、headless 后端                                        |
| 系统集成   | **全缺**                                                                | 通知、托盘、剪贴板、文件对话框、全局快捷键——**连文档都还没提过**   |
| 横切能力   | `capability` 下只有 `webview` 一个                                      | `media` / `update`；以及 `capability` 怎么并列多个能力的规矩还没立 |
...
| 命令落地   | `WorldExecutor`（方块类命令 → 标脏区块）                                   | **实体执行器**——等实体存储                                       |
| 场景与实体 | **零**；`core/scene.rs` **不存在**（模块表却说 core 职责含「场景」）       | 实体 / 组件存储：一切下游的前提，见「依赖与先后」                |
| 存档       | **未开始**                                                                 | 区块与实体的持久化、版本迁移；命令可序列化是给它留的口子         |
| 资产管线   | 只有 `data/fonts/` 落地；模型基准定为 glTF 2.0 `.glb`（见路线图）          | 本地化、模型、纹理、着色器的装载与烘焙；`data/` 其余目录仍是空的 |
...
| 缺口                                                           | 文档里的说法                                     | 代码事实                                                                        |
| -------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------- |
| 取帧 → 纹理（`import_texture` 一类）                           | webview 双线一节曾当已有接口写                   | 全仓不存在；`create_texture_from_hal` 同样不存在（本段已改成标注）              |
| `core/scene.rs`                                                | 模块表写 core 职责含「场景」                     | 文件不存在；`core/` 下只有 command / event / geometry / input / window / widget |
...
| 系统集成（通知 / 托盘 / 剪贴板 / 文件对话框 / 全局快捷键）     | **从没提过**                                     | 代码全无——属于「连规划都没写」的那一类                                          |
...
```text
实体 / 组件存储 ──┬─→ 实体命令执行器（WorldExecutor 的下一档）
                  ├─→ 存档（区块 + 实体）
                  ├─→ 动画（骨骼要绑到实体上）
                  └─→ 脚本 / 插件绑定（「操作谁」得先有个谁）

gpu 档（设备与取帧，已有）──┐
纹理导入接口（缺）··········┼─→ CEF 纹理模式 ──→ 覆盖层 / 纹理双线齐备
pump 钩子（接口有，语义未检验）┘

求值目标（字节码 / AST / WIT 组件）──→ 代码语言与积木语言 ──→ 脚本插件边界 ──→ 插件宿主

ui 裁剪 + 流式布局 ─── 不依赖任何其它能力
系统集成 / IME ─────── 不依赖任何其它能力
```

由此得到的先后（**只排依赖，不排工期**）：

1. **实体 / 组件存储**先做——它解锁的下游最多（命令落地、存档、动画、脚本绑定全等它），也是唯一
   一个「不做就卡住一片」的
2. **`ui` 裁剪 / 流式布局**与**系统集成 / IME** 跟上面**互不依赖**，可以并行插进来；编辑器那条线的
   「拖放组合容器」只等裁剪，而裁剪是 `ui` 内部的事，不欠别的能力
3. **纹理导入接口**是 CEF 纹理模式三件前置里唯一缺的（设备与取帧 `gpu` 档已经有了）。所以那一步的
   入口不是「先引 cef」，而是**先把「取帧变成纹理」的接口定下来**——它只要 `gpu` 档，不必等 `render`
4. **先定求值目标，再定插件边界，最后才谈插件宿主**。反过来做（先挑 wasmtime、先写 C ABI）会返工：
   边界形状由 IR 与求值器决定，不由宿主运行时决定
5. 资产管线、存档、音频、动画、联机是**各自独立的长线**，互不为前置；谁先做看应用先需要什么
